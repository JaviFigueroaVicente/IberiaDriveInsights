...
import Swal from 'sweetalert2' // Importación crítica para el modal de respuesta
...
export default function Predict() {
    ...
    const handlePredict = async (e) => {
        ...
        try {
            ...
            // Supongamos que tu backend devuelve { price: 15400 } o un campo similar como 'estimated_price'
            const predictedPrice = result?.price || result?.estimated_price || 0;

            // 2. Interceptar el flujo para inyectar el modal de evaluación técnica
            await Swal.fire({
                ...swalConfig,
                ...
                html: `
                    <div style="font-family: monospace; text-align: left; margin: 8px 0; padding: 8px;">                        
                        <div style="background: rgba(0, 0, 0, 0.4); padding: 20px; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 4px; text-align: center; margin: 16px 0; position: relative;">
                            ...
                            <span style="font-size: 32px; font-weight: 800; letter-spacing: -0.025em; color: var(--secondary, #5de6ff); text-shadow: 0 0 12px rgba(93, 230, 255, 0.2);">
                                ${predictedPrice} €
                            </span>
                        </div>
    ...
    return (
        <div className="bg-(--surface) text-[#dae2fd] p-6 lg:p-12 blueprint-grid relative overflow-hidden flex items-center justify-center mb-6">
                    ...
                    <div className="lg:col-span-8 bg-(--surface-low) p-8 md:p-12 border-r border-white/5 space-y-12">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {/* Selector de Fabricante (Siempre habilitado) */}
                                <div className="space-y-2">
                                ...
                                <div className={`space-y-2 transition-opacity duration-200 ${!formData.make ? 'opacity-40' : 'opacity-100'}`}>
                                    ...
                                    <select 
                                        ...
                                        className={`input-data-entry w-full ${!formData.make ? 'cursor-not-allowed' : ''}`}
                                    >
                                ...
                                <div className={`md:col-span-2 space-y-2 transition-opacity duration-200 ${(!formData.make || !formData.model) ? 'opacity-40' : 'opacity-100'}`}>
                                    ...
                                    <select 
                                        ...
                                        className={`input-data-entry w-full ${(!formData.make || !formData.model) ? 'cursor-not-allowed' : ''}`}
                                    >
                        ...
                        <div className="space-y-8">
                            <div className="pb-6 border-b border-white/10">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between text-[11px]">
                                        ...
                                        <span className="text-white font-mono">{formData.make ? 'DATOS RECUPERADOS' : 'ESPERANDO...'}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-black/40 p-5 rounded-sm border border-white/5">
                                <p className="text-[10px] leading-relaxed text-[#bec8d2]/60 uppercase tracking-wider mb-4">
                                    El cálculo se basará en el análisis sobre {selectedVersion.nombre || 'el modelo seleccionado'}.
                                </p>
