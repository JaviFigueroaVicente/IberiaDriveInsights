...
export default function UserRegistry({ currentUser, handleLogout }) {
    ...
    const openModal = (user = null) => {
        if (user) {
            setEditingUser(user);
            setBackupPassword(user.password || ""); // Resguardamos el hash hash/string antiguo
            setFormData({
                email: user.email,
                password: "", // El input visual empieza limpio
                name: user.name || "",
    ...
    // Gestión del envío del formulario (Creación / Edición) Envia todos los campos requeridos por Pydantic
    const handleSubmit = async (e) => {
        e.preventDefault();

        try {
            if (editingUser) {
                // Generar payload riguroso mapeado 1:1 con el Pydantic UserUpdate
                const updatePayload = {
                    email: formData.email,
                    name: formData.name,
                    surname: formData.surname,
                    role: Number(formData.role),
                    // Si el input está vacío, remite la contraseña antigua guardada en el estado
                    password: formData.password.trim() !== "" ? formData.password : backupPassword,
                    updated_at: new Date().toISOString() // Sello de tiempo exigido por el esquema del backend
                };

                const updatedData = await updateUser(editingUser.id, updatePayload);
                setUsers(users.map(u => u.id === editingUser.id ? { ...u, ...updatedData } : u));
                Toast.fire({ icon: 'success', title: 'IDENTIDAD_ACTUALIZADA_BBDD' });
            } else {
                // Validación estricta de contraseña en creación
                if (!formData.password) {
                    Toast.fire({ icon: 'error', title: 'CONTRASENA_REQUERIDA' });
                    return;
                }
            ...
            Toast.fire({ icon: 'error', title: 'ERROR_PROTOCOLO_AUTH_422' });
        }
    };
    ...
    const handleDeleteUser = async (user) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                Toast.fire({ icon: 'success', title: 'NODO_REVOCADO_SISTEMA' });
            } catch (error) {
                console.error("Error al eliminar identidad:", error);
                Toast.fire({ icon: 'error', title: 'FALLO_AL_PURGAR_ENTIDAD' });
            }
        }
    };
                                ...
                                <form onSubmit={handleSubmit} className="p-5 sm:p-8 max-h-[90vh] overflow-y-auto scrollbar-hide">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-(--surface-highest) p-2.5 text-xs text-white outline-none border border-white/5 focus:border-(--secondary) uppercase" />
                                        </div>

                                        <div>
                                            <label className="text-[9px] sm:text-[10px] font-bold text-(--secondary) uppercase mb-1.5 block">02_Apellidos</label>
                                            <input required type="text" value={formData.surname} onChange={e => setFormData({...formData, surname: e.target.value})} className="w-full bg-(--surface-highest) p-2.5 text-xs text-white outline-none border border-white/5 focus:border-(--secondary) uppercase" />
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select required value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full bg-(--surface-highest) p-2.5 text-xs text-white outline-none border border-white/5 focus:border-(--secondary)">
                                                <option value={2}>BASIC_USER (TIER_2)</option>
                                                <option value={1}>SYSTEM_OPERATOR (TIER_1)</option>
                                                <option value={0}>ROOT_ADMIN (TIER_0)</option>
                                            </select>
                                        </div>
                                    </div>
